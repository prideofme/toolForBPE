...
public class toolForBPE {
	...
	/*public static void main(String[] args) throws IOException {
		...
		String path = "D:\\scanner\\in.txt";
		//String path = "D:\\in.txt";
		tfb.name(path);
	}*/
	public void name(String path) throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(new File(path)));
		String line_origin;

		while((line_origin = br.readLine()) != null) {
			String str = line_origin.trim();
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter(new File("D:\\scanner\\out.txt")));
		bw.write(string);
		br.close();
		bw.close();
	...
	public void getFeild(String line) {
		...
		String name = str.substring(idx_name+5, idx_class).trim().replaceAll("\"","");
		String clas = str.substring(idx_class+len, idx_code).trim().replaceAll("\"","");
		if(string.contains(name) && !str.startsWith("<field")) {
			...
			String string_clas = string.substring(string_idx_clas+begin, string_idx_code).trim().replaceAll("\"","");//+++
